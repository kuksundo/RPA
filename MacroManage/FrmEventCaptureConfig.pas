...
begin
  ...
  try
    ...
    TINIConfigBase.LoadObject2Form(TForm(LEventCaptureConfigF), AEventCaptureConfig, True);

    if LEventCaptureConfigF.ShowModal = mrOK then
    begin
      TINIConfigBase.LoadForm2Object(TForm(LEventCaptureConfigF), AEventCaptureConfig, True);

      if AFileName = '' then
        AFileName := ChangeFileExt(Application.ExeName, '.ini');

      AEventCaptureConfig.Save(AFileName);
    end;
  finally
